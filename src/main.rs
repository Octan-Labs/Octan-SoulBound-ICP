...
mod types {
    ...
    /// The canister should match with the signature defined in `spec.md` in order to be considered as a sbt contract.
    #[derive(CandidType, Deserialize)]
...
#[query()]
#[candid_method(query)]
fn sbt_total_supply() -> Nat {
    ledger::with(|ledger| Nat::from(ledger.tokens_count()))
}

#[query()]
#[candid_method(query)]
fn sbt_total_transactions() -> Nat {
...
#[query()]
#[candid_method(query)]
fn sbt_total_unique_holders() -> Nat {
...
fn sbt_stats() -> Stats {
    Stats {
        total_transactions: sbt_total_transactions(),
        total_supply: sbt_total_supply(),
        cycles: sbt_cycles(),
        total_unique_holders: sbt_total_unique_holders(),
    }
}
...
#[query()]
#[candid_method(query)]
fn sbt_supported_interfaces() -> Vec<SupportedInterface> {
    vec![
...
#[query()]
#[candid_method(query)]
fn sbt_balance_of(owner: Principal) -> Result<Nat, NftError> {
    ledger::with(|ledger| {
        ledger
...
#[query()]
#[candid_method(query)]
fn sbt_owner_of(token_identifier: TokenIdentifier) -> Result<Option<Principal>, NftError> {
    ledger::with(|ledger| ledger.owner_of(&token_identifier))
}

#[query()]
#[candid_method(query)]
fn sbt_operator_of(token_identifier: TokenIdentifier) -> Result<Option<Principal>, NftError> {
    ledger::with(|ledger| ledger.operator_of(&token_identifier))
}

#[query(manual_reply = true)]
#[candid_method(query)]
fn sbt_owner_token_metadata(
    owner: Principal,
...
#[query(manual_reply = true)]
#[candid_method(query)]
fn sbt_operator_token_metadata(
    operator: Principal,
) -> ManualReply<Result<Vec<TokenMetadata>, NftError>> {
...
#[query(manual_reply = true)]
#[candid_method(query)]
fn sbt_owner_token_identifiers(
    owner: Principal,
...
#[query(manual_reply = true)]
#[candid_method(query)]
fn sbt_operator_token_identifiers(
    operator: Principal,
) -> ManualReply<Result<Vec<TokenIdentifier>, NftError>> {
...
#[query()]
#[candid_method(query)]
fn sbt_is_approved_for_all(owner: Principal, operator: Principal) -> Result<bool, NftError> {
    ledger::with(|ledger| {
        ledger
...
#[update]
#[candid_method(update)]
fn sbt_approve(operator: Principal, token_identifier: TokenIdentifier) -> Result<Nat, NftError> {
...
#[update]
#[candid_method(update)]
fn sbt_set_approval_for_all(operator: Principal, is_approved: bool) -> Result<Nat, NftError> {
    ledger::with_mut(|ledger| {
        let caller = caller();
        operator
            .ne(&caller)
